"""
Registration tests for the Artwork vector stage.

Registered Artwork geometry uses one canonical, dimensionless coordinate
system shared by vector layers and the registered Artwork envelope.
"""
# File: tests/model/artwork/test_vector_registration.py
...
def _write_raster_manifest(
    ...
    *,
    ...
    """
    Write a raster manifest for registration tests.
    """
...
def test_registered_envelope_preserves_occupied_bounds_relative_to_common_crop(
    ...
    assert float(rectangle.get("x", "nan")) == pytest.approx(10.0)
    assert float(rectangle.get("y", "nan")) == pytest.approx(10.0)
    assert float(rectangle.get("width", "nan")) == pytest.approx(40.0)
    assert float(rectangle.get("height", "nan")) == pytest.approx(60.0)
...
def test_vector_registration_preserves_common_coordinates_between_envelope_and_layers(
    ...
    assert float(envelope_rectangle.get("x", "nan")) == pytest.approx(1.0)
    assert float(envelope_rectangle.get("y", "nan")) == pytest.approx(2.0)
    assert float(envelope_rectangle.get("width", "nan")) == pytest.approx(14.0)
    assert float(envelope_rectangle.get("height", "nan")) == pytest.approx(12.0)


# =========================================================
# Registered Artwork consumer tests
# =========================================================
