...
def execute(
    ...
    try:
        ...
        if artwork_hole_diameter > 0.0:
            physical_envelope_bounds = _physical_envelope_bounds(
                envelope_bounds,
                artwork_size=artwork_size,
            )

            artwork_hole_edge_distance = float(
                context.resolver(
                    "artwork_hole_edge_distance",
                )
            )

            artwork_hole_position = int(
                context.resolver(
            ...
            hole_geometry = create_hole_geometry(
                ...
                diameter=artwork_hole_diameter,
                edge_distance=artwork_hole_edge_distance,
                position=artwork_hole_position,
            )
...
class _ArtworkTransform:
    """
    Common standalone Artwork planar dimensionalization.

    Registered Artwork geometry shares one coordinate system. The occupied
    Artwork envelope determines the uniform physical scale and translation
    applied to every standalone Artwork component derived from that
    registered geometry.
    """
    ...
    envelope_extent: float
    envelope_center_x: float
    envelope_openscad_center_y: float


def _artwork_transform(
    *,
    registered_extent: int,
    envelope_bounds: tuple[
        float,
    ...
    """
    Resolve the common planar transformation for standalone Artwork.

    The occupied registered envelope determines scale and centering.

    SVG coordinates use positive Y downward. OpenSCAD SVG import presents
    the registered geometry in an upward-positive coordinate system, so the
    registered envelope Y center is converted accordingly.

    Physical scale itself remains determined by artwork_size at the SCAD
    generation boundary.
    """
    ...
    return _ArtworkTransform(
        registered_extent=registered_extent,
        envelope_width=envelope_width,
        envelope_height=envelope_height,
        envelope_extent=envelope_extent,
        envelope_center_x=envelope_center_x,
        envelope_openscad_center_y=envelope_openscad_center_y,
    )
...
def _build_base_scad(
    ...
    *,
    ...
    The Base uses the registered Artwork envelope as its planar geometry.

    The occupied envelope determines standalone physical size. The same
    uniform scale and translation used for Artwork color layers are applied
    to the Base so that it remains registered with the dimensionalized
    Artwork.
    ...
    from the planar Base before extrusion so that it passes through the
    complete Z extent of the resulting Base.
    """

    envelope = envelope.resolve()

    if not envelope.is_file():
        raise ExtrudeError(f"Artwork envelope does not exist: {envelope}")
    ...
    base_raise_scad = _scad_number(
        base_raise,
    )

    envelope_svg = _scad_string(
        str(envelope),
    )
...
"""
    ...
    if hole_geometry is None:
        physical_planar = base_planar

    else:
        hole_center_x_scad = _scad_number(
            hole_geometry.center_x,
        )

        hole_center_y_scad = _scad_number(
            hole_geometry.center_y,
        )

        hole_radius_scad = _scad_number(
            hole_geometry.radius,
        )

        hole_planar = f"""translate(
    [
        {hole_center_x_scad},
        {hole_center_y_scad}
    ]
...
def _build_hole_scad(
    ...
    *,
    ...
    """
    Return OpenSCAD source for one subtractive Artwork Hole.

    HoleGeometry owns the Artwork-specific planar geometry and placement.
    This function translates that resolved physical geometry into a
    subtractive cylinder spanning the complete requested physical Z extent.
    """

    center_x_scad = _scad_number(
        geometry.center_x,
    )

    center_y_scad = _scad_number(
        geometry.center_y,
    )

    radius_scad = _scad_number(
        geometry.radius,
    )

    bottom_z_scad = _scad_number(
        bottom_z,
    )

    height_scad = _scad_number(
        top_z - bottom_z,
    )
...
def _build_scad(
    ...
    *,
    ...
    hole_geometry optionally supplies the resolved physical planar geometry
    of a participating Artwork Hole. When supplied, the Hole is subtracted
    through the complete Z extent of this Artwork color layer.
...
"""
    ...
    if hole_geometry is None:
        physical_solid = artwork_solid

    else:
        hole_solid = _build_hole_scad(
            hole_geometry,
            bottom_z=artwork_z,
            top_z=artwork_z + artwork_raise,
        )
...
def _build_outer_ridge_scad(
    ...
    *,
    ...
    The Outer Ridge occupies the planar region between the registered
    Artwork envelope dimensionalized at the full artwork_size and the same
    envelope uniformly scaled inward to reserve the configured ridge width.
    ...
    hole_geometry optionally supplies the resolved physical planar geometry
    of a participating Artwork Hole. When supplied, the Hole is subtracted
    from the planar Outer Ridge before extrusion so that it passes through
    the complete Z extent of the resulting Ridge.
    """
    ...
    outer_ridge_scale = artwork_scale_for_outer_ridge(
        artwork_size=artwork_size,
    ...
    outer_ridge_scale_scad = _scad_number(
        outer_ridge_scale,
    )

    outer_ridge_raise_scad = _scad_number(
        outer_ridge_raise,
    )

    outer_ridge_z_scad = _scad_number(
        outer_ridge_z,
    )
...
outer_ridge_scale = {outer_ridge_scale_scad};
outer_ridge_raise = {outer_ridge_raise_scad};
outer_ridge_z = {outer_ridge_z_scad};
