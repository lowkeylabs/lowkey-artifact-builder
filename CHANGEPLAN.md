...
Stage execution, Product identity, and persistent Product layout should be
preserved unless a specific requirement below demonstrates that a change is
necessary.
...
# Phase 0 — Review and Clarify Permanent Specifications

Before changing production behavior, review `ARCHITECTURE.md` and the applicable
Model `DEFINITION.md` files against the semantics established by the current
design.

This phase is primarily clarification of terminology and ownership. It is not
intended to introduce a new architecture.

## 0.1 Parameter Ownership

Review permanent specifications for language describing Models as directly
owning Parameters.
...
│   └── Parameters
└── Variant
    └── sparse Feature-parameter overrides
```

A Model owns Features and Variants.

A Feature owns the Parameters that configure its behavior.

A Variant is a named, Model-scoped sparse set of overrides to Parameters owned
by Features of that Model.

Feature Parameter defaults establish ordinary Feature behavior.

A Variant changes the resulting Model behavior by overriding selected Feature
Parameters. It does not contain Features, independently select Features, or
define Feature semantics.

Feature participation is determined by the semantics of the applicable Feature
and the effective values of its Parameters.

Review `ARCHITECTURE.md` and Model definitions for phrases such as:
...
and clarify them where they incorrectly imply direct Model ownership of
Parameters.

Do not mechanically change implementation names merely to reproduce this
conceptual hierarchy.

## 0.2 Preserve Variant and Realization Semantics

Confirm that the permanent specifications continue to express:

```text
Variant
    = Model-scoped named sparse Feature-parameter overrides

Realization
    = application of a Variant to an Artifact
      + optional Artifact-specific customization
```
...
## 0.3 Preserve Product Architecture
...
Phase 0 is complete when the permanent specifications accurately express the
intended ownership and identity semantics needed by the following phases.

Prefer small wording corrections over architectural restructuring.
...
This is a specification correction followed by a bug fix.
...
desired physical size of the actual Artwork.

## 1.1 Define Artwork Size

Update the Artwork permanent specification before changing tests or production
implementation.
...
* a dependency target;
* a replacement for the canonical Product;
...
Qualified Variant configuration should continue to produce the same conceptual
resolution order:

```text
Feature Parameter defaults
        ↓
Variant sparse parameter overrides
        ↓
...
The schema change should not introduce a second Variant identity mechanism.
...
* Artifact-specific Feature Parameter overrides;
...
Parameters belong to Features.

Variants added later will configure these Features by supplying sparse
Parameter overrides.
...
The conceptual loop is a cylindrical/ring attachment controlled by Feature
...
* how loop participation is determined;
...
Phase 4 is complete when the required Model-owned Features and their
Feature-owned Parameters are specified, tested, and available for configuration
independently of any particular specialized Variant.
...
A Variant supplies sparse overrides to Parameters owned by Features of its
Model.
...
Each Variant should specify only the Feature Parameter values that distinguish
it from ordinary Artwork behavior.
...
values without redefining the loop Feature or repeating unrelated Feature
defaults.
...
named reusable configuration of already-supported Model Features.
...
* intended sparse Feature Parameter overrides;
* inheritance of unspecified Feature Parameter defaults;
...
Model Features through sparse Feature Parameter overrides and can be selected
and built through the existing Variant workflow.
...
1. permanent specifications clearly express Model, Feature, Parameter, Variant,
   and Realization ownership and relationships;
...
5. the required Model-owned Features and Feature-owned Parameters are
   implemented according to their Model definitions;
6. the desired Variants configure those Features through sparse Parameter
   overrides;
...
> Clarify the semantics, correct the behavior, and extend the existing
> architecture only as much as the requested capabilities require.
